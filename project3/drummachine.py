...
class DrumMachine:
    ...
    def load_project(self):
        file_path = filedialog.askopenfilename(
            filetypes=[('Explosions Beat File', '*.ebt')], title='Load Project')
        if not file_path:
            return
    ...
    def on_loop_button_toggle(self):
    ...
    def create_play_bar(self):
        ...
        self.play_button = ttk.Button(playbar_frame, text='Play',  image=self.play_icon,
        ...
        ttk.Checkbutton(playbar_frame, text='Loop', variable=self.to_loop,
                    command=self.on_loop_button_toggled).grid(row=start_row, column=16, padx=5)
        ttk.Label(playbar_frame, text='Beats Per Minute').grid(
        ...
        label = ttk.Label(playbar_frame, image=photo)
        label.image = photo
    ...
    def create_left_drum_loader(self):
        ...
        for i in range(MAX_NUMBER_OF_DRUM_SAMPLES):
            open_file_button = ttk.Button(left_frame, image=open_file_icon,
            ...
            self.drum_load_entry_widget[i] = ttk.Entry(left_frame)
    ...
    def create_top_bar(self):
        ...
        ttk.Label(topbar_frame, text='Pattern Number:').grid(row=0, column=1)
        ...
        self.current_pattern_name_widget = ttk.Entry(topbar_frame)
        self.current_pattern_name_widget.grid(row=0, column=3, padx=7, pady=2)

        ttk.Label(topbar_frame, text='Number of Units:').grid(row=0, column=4)
        ...
        ttk.Label(topbar_frame, text='BPUs:').grid(row=0, column=6)
